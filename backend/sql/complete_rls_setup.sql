...
ALTER TABLE "public"."MilestoneProgress" ENABLE ROW LEVEL SECURITY;
...
WITH CHECK (
    EXISTS (
        ...
        WHERE id = "teamId" AND "creatorId" = auth.uid()
    )
);

-- Users can leave teams they belong to
CREATE POLICY "Users can leave teams they belong to" 
ON "public"."TeamMembership" 
FOR DELETE 
...
CREATE POLICY "Challenge creators can create milestones" 
ON "public"."Milestone" 
FOR INSERT
WITH CHECK (
    EXISTS (
        SELECT 1 FROM "public"."Challenge" 
        WHERE id = "challengeId" AND "creatorId" = auth.uid()
    )
);

-- Challenge creators can update milestones for their challenges
CREATE POLICY "Challenge creators can update milestones" 
ON "public"."Milestone" 
FOR UPDATE
USING (
    EXISTS (
...
CREATE POLICY "Challenge creators can delete milestones" 
ON "public"."Milestone" 
FOR DELETE 
USING (
    EXISTS (
...
CREATE POLICY "Milestone progress is viewable by everyone" 
ON "public"."MilestoneProgress" 
FOR SELECT
USING (true);

-- Participants can update their own milestone progress
CREATE POLICY "Participants can update their milestone progress" 
ON "public"."MilestoneProgress" 
FOR INSERT
WITH CHECK (
    EXISTS (
        SELECT 1 FROM "public"."ChallengeParticipant" 
        WHERE id = "participantId" AND "userId" = auth.uid()
    )
);

-- Participants can update their own milestone progress
CREATE POLICY "Participants can update their own milestone progress" 
ON "public"."MilestoneProgress" 
FOR UPDATE
USING (
    EXISTS (
...
CREATE POLICY "System can manage milestone progress" 
ON "public"."MilestoneProgress" 
FOR ALL
USING (true)
WITH CHECK (true);
...
ON "public"."ChallengeParticipant" 
FOR SELECT
USING (true);

-- Users can join challenges as individuals
CREATE POLICY "Users can join challenges individually" 
ON "public"."ChallengeParticipant" 
FOR INSERT
WITH CHECK (auth.uid() = "userId" AND "teamId" IS NULL);

-- Team members can join challenges as teams (if they belong to the team)
CREATE POLICY "Team members can join challenges as teams" 
ON "public"."ChallengeParticipant" 
FOR INSERT
WITH CHECK (
    ...
    "userId" IS NULL AND
    EXISTS (
        SELECT 1 FROM "public"."TeamMembership" 
        WHERE "teamId" = "ChallengeParticipant"."teamId" AND "userId" = auth.uid()
    )
);

-- Users can leave challenges they joined individually
CREATE POLICY "Users can leave individual challenges" 
ON "public"."ChallengeParticipant" 
FOR DELETE 
USING (auth.uid() = "userId" AND "teamId" IS NULL);

-- Team members can remove their team from challenges
CREATE POLICY "Team members can remove teams from challenges" 
ON "public"."ChallengeParticipant" 
FOR DELETE 
USING (
    "teamId" IS NOT NULL AND 
...
CREATE POLICY "Users can create posts for their participations" 
ON "public"."Post" 
FOR INSERT
WITH CHECK (
    EXISTS (
