...
WITH
    CHECK (
        EXISTS (
            ...
            WHERE
                cp."challengeId" = "discussion_posts"."challengeId" AND
                cp."userId" = auth.uid ()
        ) AND
        NOT EXISTS (
            SELECT
            ...
            WHERE
                db."challengeId" = "discussion_posts"."challengeId" AND
                db."userId" = auth.uid () AND
                db."isActive" = true AND
                (
                    db."expiresAt" IS NULL OR
                    db."expiresAt" > NOW()
                )
        )
    );
...
UPDATE USING (
    ...
    EXISTS (
        ...
        WHERE
            dm."challengeId" = "discussion_posts"."challengeId" AND
            dm."userId" = auth.uid ()
    ) OR
    EXISTS (
        SELECT
        ...
        FROM
            "public"."challenges" c
        WHERE
            c.id = "discussion_posts"."challengeId" AND
            c."creatorId" = auth.uid ()
    )
);
...
WITH
    CHECK (
        EXISTS (
            ...
            WHERE
                dp.id = "discussion_replies"."postId" AND
                cp."userId" = auth.uid ()
        ) AND
        NOT EXISTS (
            SELECT
...
CREATE POLICY "Challenge creators can manage moderators" ON "public"."discussion_moderators" FOR ALL USING (
    EXISTS (
        ...
        FROM
            "public"."challenges" c
        WHERE
            c.id = "discussion_moderators"."challengeId" AND
            c."creatorId" = auth.uid ()
    )
);
    ...
    USING (
        EXISTS (
            ...
            FROM
                "public"."discussion_moderators" dm
            WHERE
                dm."challengeId" = "discussion_bans"."challengeId" AND
                dm."userId" = auth.uid ()
        ) OR
        EXISTS (
            SELECT
            ...
            FROM
                "public"."challenges" c
            WHERE
                c.id = "discussion_bans"."challengeId" AND
                c."creatorId" = auth.uid ()
        )
    );
