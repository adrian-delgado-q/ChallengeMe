...
export const CommonValidationSchemas = {
    teamName: (value: string) => ValidationUtils.combine(
        ValidationUtils.required(value, 'Team name'),
        ValidationUtils.textLength(value, 3, 50, 'Team name')
    ),

    challengeTitle: (value: string) => ValidationUtils.combine(
    ...
    maxParticipants: (value: string) => value.trim() ? 
        ValidationUtils.numeric(value, 1, 1000, 'Max participants') : 
        { isValid: true }
};
