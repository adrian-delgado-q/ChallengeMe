			...
			tooltip: {
				callbacks: {
					afterLabel: function (context: any) {
						...
						const relevantPoint = progressData.find(
							p => p.activityType === activityTypeId && p.date === date
						);

						if (relevantPoint && relevantPoint.milestoneLevel > 0) {
							const milestone = milestones.find(
								m => m.activityTypeId === activityTypeId && m.order === relevantPoint.milestoneLevel
							);
							return milestone
								? `🏆 Milestone: ${milestone.name}`
								: `🏆 Milestone Level ${relevantPoint.milestoneLevel}`;
						}
						return '';
					},
				},
	...
	if (progressData.length === 0) {
		return (
			<Card p={6}>
				...
				<Center h="250px">
					<Text color="gray.500">
						No progress data available yet. Start logging activities to see your progress!
					</Text>
				</Center>
			</Card>
		);
