...
export const DiscussionModeration: React.FC<DiscussionModerationProps> = ({ challengeId }) => {
	...
	return (
		<>
			<Box
				bg={moderatorBg}
				borderColor={moderatorBorderColor}
				borderWidth="1px"
				borderRadius="md"
				p={2}
			>
				<HStack justify="space-between" align="center">
					<HStack spacing={2} fontSize="sm">
						<BsShield size="14px" />
						<Text fontWeight="semibold" color={moderatorTextColor}>
							Moderator
						</Text>
					...
					<Button size="xs" colorScheme="orange" variant="outline" onClick={onOpen}>
						Manage
					</Button>
				</HStack>
			</Box>
...
const BannedUserItem: React.FC<BannedUserItemProps> = ({ ban, onUnban, canUnban }) => {
	...
	return (
		<Box p={4} borderWidth="1px" borderColor="gray.200" borderRadius="md" bg="white">
			<HStack justify="space-between" align="center">
				<HStack spacing={3}>
					<Avatar src={ban.user.avatarUrl || ban.user.avatar_url} name={ban.user.username} size="sm" />
