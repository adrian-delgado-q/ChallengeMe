							...
							<HStack spacing={3}>
								...
								{canManageRoles && member.userId !== currentUser?.id ? (
									<Select
										value={member.role}
										onChange={e =>
											handleRoleChange(member.id, member.userId, e.target.value as 'ADMIN' | 'MEMBER')
										}
										size="sm"
										width="120px"
			...
			<AlertDialog isOpen={isOpen} leastDestructiveRef={cancelRef} onClose={onClose}>
				<AlertDialogOverlay>
					<AlertDialogContent>
						...
						<AlertDialogBody>
							Are you sure you want to remove <strong>{memberToRemove?.user?.username}</strong> from the
