    ...
    <VStack spacing={3} align="stretch" flex="1">
      ...
      <VStack spacing={2} align="stretch" fontSize="sm" color="gray.600">
        <HStack>
          ...
          <Text>Final Goal: {challenge.milestones && challenge.milestones.length > 0
            ? challenge.milestones[challenge.milestones.length - 1]?.value + ' points'
            : 'No milestones set'}</Text>
        </HStack>
        <HStack>
          <Icon as={UserTeamIcon} w={5} h={5} color="blue.400" />
          <Text>{challenge.participants || 0} {challenge.challengeType === 'team' ? 'Teams' : 'Participants'}</Text>
        </HStack>
        <HStack>
