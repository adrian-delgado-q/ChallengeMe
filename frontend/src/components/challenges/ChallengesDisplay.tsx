			...
			<ul>
				{data?.challenges?.map(challenge => (
					<li key={challenge.id} style={{ border: '1px solid #ccc', margin: '10px', padding: '10px' }}>
						...
						<p>
							<strong>Creator:</strong> {challenge.creator?.username}
						</p>
						<p>
						...
						<p>
							<strong>Participants:</strong> {challenge.participant_count} /{' '}
							{challenge.max_participants || 'Unlimited'}
						</p>
						<p>
							<strong>Dates:</strong> {challenge.start_date} to {challenge.end_date}
						</p>
						<div>
							...
							<ul>
								{challenge.milestones?.map(milestone => (
									<li key={milestone.id}>
										{milestone.name} - Target: {milestone.target_value} {milestone.activity_type?.unit_label}
									</li>
								))}
							</ul>
						</div>
