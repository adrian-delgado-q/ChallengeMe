...
export const ActivityTypeSelector: React.FC<ActivityTypeSelectorProps> = ({
	...
	const handleKeyDown = (e: React.KeyboardEvent) => {
		...
		switch (e.key) {
			case 'ArrowDown':
				e.preventDefault();
				setHighlightedIndex(prev => (prev < filteredActivityTypes.length - 1 ? prev + 1 : prev));
				break;
			case 'ArrowUp':
				e.preventDefault();
	...
	const groupedActivityTypes = filteredActivityTypes.reduce(
		(acc, activityType) => {
	...
	return (
		<VStack spacing={4} align="stretch">
			...
			<Text fontSize="xs" color="gray.500">
				Select one or more activities for this challenge. Participants can log activities from any of
				the selected types.
			</Text>
		</VStack>
	);
