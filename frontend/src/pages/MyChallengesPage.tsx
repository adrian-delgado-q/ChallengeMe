				...
				<Card p={6} bg={cardBg} borderColor={borderColor}>
					...
					<SimpleGrid columns={{ base: 2, md: 4 }} spacing={6}>
						...
						<Stat>
							...
							<StatNumber>{challenges.filter(c => c.status === 'ACTIVE' || !c.status).length}</StatNumber>
						</Stat>
						<Stat>
						...
						<Stat>
							...
							<StatNumber>{challenges.reduce((sum, c) => sum + c.recentActivitiesCount, 0)}</StatNumber>
						</Stat>
					</SimpleGrid>
				</Card>
			)}
				...
				<Card p={12} textAlign="center" bg={cardBg} borderColor={borderColor}>
					<VStack spacing={6}>
						...
						<Text color="gray.600" maxW="md">
							You haven't created any challenges yet. Create your first challenge to start building a
							community around your fitness goals.
						</Text>
						<Button
							leftIcon={<AddIcon />}
							colorScheme="orange"
								...
								<HStack justify="space-between" align="start">
									...
									<Menu>
										...
										<MenuList>
											...
											<MenuItem icon={<ViewIcon />} onClick={() => navigate(`/challenges/${challenge.id}`)}>
												View Public Page
											</MenuItem>
										</MenuList>
									</Menu>
