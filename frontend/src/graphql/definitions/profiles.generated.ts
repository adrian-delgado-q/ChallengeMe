...
export const useGetProfileQuery = <TData = GetProfileQuery, TError = unknown>(
	...
	options?: Omit<UseQueryOptions<GetProfileQuery, TError, TData>, 'queryKey'> & {
...
export const useGetProfilesQuery = <TData = GetProfilesQuery, TError = unknown>(
	dataSource: { endpoint: string; fetchParams?: RequestInit },
	variables?: GetProfilesQueryVariables,
	options?: Omit<UseQueryOptions<GetProfilesQuery, TError, TData>, 'queryKey'> & {
		queryKey?: UseQueryOptions<GetProfilesQuery, TError, TData>['queryKey'];
	}
) => {
	return useQuery<GetProfilesQuery, TError, TData>({
		queryKey: variables === undefined ? ['GetProfiles'] : ['GetProfiles', variables],
		queryFn: fetcher<GetProfilesQuery, GetProfilesQueryVariables>(
			dataSource.endpoint,
			dataSource.fetchParams || {},
			GetProfilesDocument,
			variables
